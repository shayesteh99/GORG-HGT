...
for i in $(seq 1 2 69); do 
    ...
    # # echo $ialpha
    python evolve.py `echo $i/100 | bc -l` $alpha $g
    python compute_GND_AAD.py simulated_genomes/$g/mutated_BLOSUM62_${g}_a${ialpha}\_aad$(printf "%03d" ${i}) simulated_genomes/$g/$g
done
done
